...
exports.getMockData = function(apiContext, req) {
    ...
    if(apiContext) {
        throw 'Missing API context. Need a valid service context like api name, api method to get mock data';
    }
        ...
        if((apiList[i].serviceName === reqServiceName) && (apiList[i].apiName === reqApiName)) {
            ...
            for(var x in apiReq) {
                ...
                if(Array.isArray(apiReq[x])) {
                    throw "Arrays are not allowed in request!";
                }

                if(dataType === 'object') {
                    throw "Nested objects are not allowed in request!";
                }
